...
float difmap_noise, fg_scale, final_chi2, final_H, beam_fwhm, beam_freq, beam_cutoff, alpha_start, eta;

dim3 threadsPerBlockNN;
dim3 numBlocksNN;

int threadsVectorReduceNN, blocksVectorReduceNN, crpix1, crpix2, nopositivity = 0, verbose_flag = 0, clip_flag = 0, apply_noise = 0, it_maximum, status_mod_in;
 ...
 //Check peer access if there is more than 1 GPU
  ...
  free(input_Inu_0);
    ...
    for(int f = 0; f<data.nfields; f++){
  		for (int i = 0;  i < data.total_frequencies; i++) {
  			cudaSetDevice((i%num_gpus) + firstgpu);
        gpuErrchk(cudaMalloc((void**)&vars_per_field[f].device_vars[i].device_Inu, sizeof(cufftComplex)*M*N));
  ...
  gpuErrchk(cudaMemset(device_S, 0, sizeof(float)*M*N));
    ...
    for(int f = 0; f < data.nfields; f++){
  		for (int i = 0;  i < data.total_frequencies; i++) {
  			...
  			gpuErrchk(cudaMemset(vars_per_field[f].device_vars[i].device_Inu, 0, sizeof(cufftComplex)*M*N));
