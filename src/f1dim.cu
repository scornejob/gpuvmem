...
extern long M;
extern long N;
extern float MINPIX, minInu_0, S_multiplier;
...
__host__ float f1dim(float x)
{
    ...
    //xt = pcom+x*xicom;
    if(nopositivity == 0){
      evaluateXt<<<numBlocksNN, threadsPerBlockNN>>>(device_xt, device_pcom, device_xicom, x, N, minInu_0);
      gpuErrchk(cudaDeviceSynchronize());
    }else{
      if(reg_term==3){
        evaluateXtNoPositivityS<<<numBlocksNN, threadsPerBlockNN>>>(device_xt, device_pcom, device_xicom, x, N, minInu_0, S_multiplier);
        gpuErrchk(cudaDeviceSynchronize());
      }else{
